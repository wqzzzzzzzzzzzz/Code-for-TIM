    ...
    un = [uu(:,i);conj(uu(:,i))];
        ek_Rand = dn - (w_RLS_New)' * un;
         ...
         R2=0;
            ...
            Hn=Cn1*uu(:,i)+Dn1*conj(uu(:,i));
            kn= 1/(lamda_New+2*R1*real(uu(:,i)'*Hn))*Hn;
Cn1=1/  lamda_New*(Cn1-R1*Hn*kn');
Dn1=1/  lamda_New*(Dn1-R1*Hn*kn.');
w_RLS_New= w_RLS_New+[kn;conj(kn)] * (R1*conj(ek_Rand)-R2);
...
legend('Simulation (WL-RMCL)','Theoretical Prediction (WL-RMCL)');
