...
w_RLS_New2= w_RLS_New2+[kn2;conj(kn2)] * (R1*conj(ek_Rand2)-R2);
       ...
       Err_TH_RLS_New(mm) = (1-lamda_New)*p*2/ mean(diag([uu;conj(uu)]*[uu;conj(uu)]'/L))/aa/ (1+lamda_New1);   
              Err_TH_RLS_New_2(mm) = (1-lamda_New2)*p*2/ mean(diag([uu;conj(uu)]*[uu;conj(uu)]'/L))/aa1/ (1+lamda_New1);   
end
toc
...
legend('ACLMS (\eta=0.01)','WL-RLS (\lambda=0.99)','WL-MCCC (\eta=0.02,\sigma=5)','WL-RMCL (K_R=2,K_I=1,\lambda=0.99)','WL-RMCL (K_R=4,K_I=1,\lambda=0.99)','TH (WL-RMCL,K_R=2,K_I=1,\lambda=0.99)','TH (WL-RMCL,K_R=4,K_I=1,\lambda=0.99)');
