...
%     wo1=[wo0];
    ...
    w_RLS_New=zeros(p,1)+j*zeros(p,1);
    ...
    Err_TH_RLS_New=[Err_TH_RLS_New (1-lamda_New)*p/ mean(diag(uu*uu'/L))/aa/ (1+lamda_New)];
...
xlabel('\lambda');ylabel('SS-MSD(dB)');
box on;
grid on;
xlim([0.960,1]);
ylim([-18.5,0]);
